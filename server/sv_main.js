(() => {
    ...
    FYX.Procedures.register("str-mechanics:server:clockIn", async (src, pBiz) => {
        let cid = exports["str-lib"].getCharacter(src).id
        let isEmployed = exports["str-business"].IsEmployedAt(src, cid, pBiz)
        if (isEmployed) {
            FYX.Events.emitNet("str-mechanics:client:clockedInState", true)
            return true, "You Clocked in"
        }
    })

    FYX.Procedures.register("str-mechanics:server:clockOut", async (src, pBiz) => {
        let cid = exports["str-lib"].getCharacter(src).id
        let isEmployed = exports["str-business"].IsEmployedAt(src, cid, pBiz)
        if (isEmployed) {
            FYX.Events.emitNet("str-mechanics:client:clockedInState", false)
